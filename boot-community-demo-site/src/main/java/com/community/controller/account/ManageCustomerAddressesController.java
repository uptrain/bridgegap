...
public class ManageCustomerAddressesController extends BroadleafManageCustomerAddressesController {
	...
	/*@Autowired
    protected BridgeGapDao bridgeGapDao;*/
    ...
    public String viewCustomerAddresses(HttpServletRequest request, Model model) {
    	List<CustomerAddress> addresses = super.populateCustomerAddresses();
        CustomerAddress customerAddress = addresses.isEmpty() ? null : addresses.get(0);
        if (customerAddress != null) {
        	validateCustomerOwnedData(customerAddress); 
        }
        List<BridgeGapState> bgStates = populateBGStates();
        ...
        model.addAttribute("bgStates", bgStates);
        return getCustomerAddressesView();
    }
    
   /* private List<BridgeGapState> populateBGStates() {
		return bridgeGapDao.populateBGStates();
	}*/
